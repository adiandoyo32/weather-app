...
export const getGeoPosition = (lat: number, lng: number) => {
  ...
  const q = `${lat},${lng}`;
  return fetch(
    `http://dataservice.accuweather.com/locations/v1/cities/geoposition/search?q=${q}&apikey=${apiKey}`
  )
    .then((res) => res.json())
...
export const getCityByName = (cityName: string) => {
  ...
  const q = cityName
  return fetch(
    `http://dataservice.accuweather.com/locations/v1/cities/search?q=${q}&apikey=${apiKey}`
  )
    .then((res) => res.json())
    .then((data) => data as Location[]);
};
